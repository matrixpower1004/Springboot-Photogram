...
public interface SubscribeRepository extends JpaRepository<Subscribe, Integer>{
	...
	int mSubscribeState(@Param("principalId") int principalId, @Param("pageUserId") int pageUserId); // 구독된 상태라면 state = 1
	
	@Query(value = "SELECT COUNT(*) FROM subscribe WHERE fromUserId = :pageUserId", nativeQuery = true)
	int mSubscribeCount(@Param("pageUserId") int pageUserId); // 해당 페이지의 주인이 구독한 구독자 수
}
